...
void Runtime::resetMemory(Memory* memory, IR::MemoryType& newType, const std::vector<U8>& memImage) {
	...
	Lock<Platform::Mutex> resizingLock(memory->resizingMutex);
	memcpy(memory->baseAddress, memImage.data(), memImage.size());
	resizingLock.unlock();
}

void Runtime::getMemoryImage(Memory* memory, std::vector<U8>& memImage) {
    Lock<Platform::Mutex> resizingLock(memory->resizingMutex);
	const Uptr numPages = memory->numPages.load(std::memory_order_acquire);
    memImage.resize(numPages * IR::numBytesPerPage);
	memcpy(memImage.data(), memory->baseAddress, numPages * IR::numBytesPerPage);
	resizingLock.unlock();
}
