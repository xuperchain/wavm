...
static std::unique_ptr<xchain::XChainServiceClient> kXchainServiceClient;
...
void read_memory_ref(Emscripten::Instance* instance,
        const U32 start, const U32 size, char* dest) {
    if(size == 0) {
        return;
    }
    U8* emscriptenMemoryBaseAddress = getMemoryBaseAddress(instance->memory);
    auto stringMemory = emscriptenMemoryBaseAddress + start;
    memcpy(dest, stringMemory, size);
    dest[size] = 0;
}
...
DEFINE_INTRINSIC_FUNCTION(env, "__builtin_add_gas", void, add_gas, I64 gas)
{
    ...
    if(gasUsed + gas > gasLimit) {
        //trap this error
		throwException(Runtime::ExceptionTypes::outOfGas);
    }
    gasUsed += gas;
}

void Emscripten::resetMemories(Emscripten::Instance* instance, const IR::Module& module, const std::vector<U8>& memImage) {
    MemoryType memoryType(false, SizeConstraints{0, 0});
    if(module.memories.imports.size() && module.memories.imports[0].moduleName == "env"
            && module.memories.imports[0].exportName == "memory") {
        memoryType = module.memories.imports[0].type;
    ...
    Runtime::resetMemory(instance->memory, memoryType, memImage);
}

void Emscripten::getMemoryImage(Emscripten::Instance* instance, std::vector<U8>& memImage) {
    Runtime::getMemoryImage(instance->memory, memImage);
}

void xchain::init_client(xchain::XChainServiceClient* client) {
    kXchainServiceClient.reset(client);
}
